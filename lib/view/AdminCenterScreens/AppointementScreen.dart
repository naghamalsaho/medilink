...
class AppointmentsPage extends StatelessWidget {
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          ...
                          children: List.generate(ctrl.todayAppts.length, (i) {
                            ...
                            final patient =
                                (ap['patient_name'] ?? ap['patient'] ?? '-')
                                    .toString();
                            final doctor =
                                (ap['doctor_name'] ?? ap['doctor'] ?? '-')
                                    .toString();
                            final time =
                                (ap['time'] ?? ap['requested_time'] ?? '-')
                                    .toString();
                            final specialty =
                                (ap['specialty'] ?? '-').toString();
                            final notes =
                                (ap['notes'] ?? ap['reason'] ?? '').toString();
                            ...
                            return Column(
                              children: [
                                ListTile(
                                  ...
                                  isThreeLine:
                                      notes.isNotEmpty || specialty.isNotEmpty,
                                  trailing: Column(
                ...
                ElevatedButton.icon(
                  onPressed: () async {
                    ...
                    try {
                      ...
                      Get.snackbar(
                        'Error',
                        'Failed to fetch today appointments',
                      );
                    }
                  },
                                              ...
                                              actions: [
                                                ...
                                                if ((ap['status']
                                                  ...
                                                  TextButton(
                                                    onPressed: () {
                                                      print(
                                                        ' I clicked Approve on the request id=$id',
                                                      );
                                                      ctrl.approve(id);
                                                    },
  ...
  void _showEditDialog(
    ...
    final DoctorController doctorCtrl =
        Get.isRegistered<DoctorController>()
    ...
    try {
      ...
      selectedDoctorId =
          d is int ? d : (d != null ? int.tryParse(d.toString()) : null);
    } catch (_) {
    ...
    try {
      final rawDate =
          (ap['appointment_date'] ?? ap['requested_date'] ?? '').toString();
    ...
    String currentAttendance =
        (ap['attendance_status'] ?? 'present').toString();
    final notesCtrl = TextEditingController(
      text: ap['notes']?.toString() ?? '',
    );
                  ...
                  children: [
                    docs.isEmpty
                        ...
                        : DropdownButtonFormField<int>(
                          ...
                          items:
                              docs.map<DropdownMenuItem<int>>((d) {
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged:
                          (v) => setState(
                            () => currentAttendance = v ?? currentAttendance,
                          ),
                    ),
                    const SizedBox(height: 8),
                    TextField(
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    if (currentStatus.isEmpty || currentAttendance.isEmpty) {
                      Get.snackbar(
                        'Error',
                        'Please select status and attendance',
                      );
                      return;
                    }
                    ...
                    print(
                      '[UI] Updating appointment id=$appointmentId doctor=$selectedDoctorId date=$dateStr status=$currentStatus attendance=$currentAttendance notes=${notesCtrl.text}',
                    );
                    ...
                    await ctrl.updateAppointment(
                      ...
                      patientId:
                          ap['patient_id'] is int
                              ? ap['patient_id'] as int
                              : (ap['patient_id'] != null
                                  ? int.tryParse(ap['patient_id'].toString())
                                  : null),
                    );
